...
SimRacingController::SimRacingController(unsigned long powerSaveTimeoutMs) :
    ...
    numRows(0), numCols(0),
    rowPins(nullptr), colPins(nullptr),
    matrixDebounceDelay(50),
    gpioPins(nullptr), lastGpioStates(nullptr), gpioStates(nullptr),
    gpioDebounceTime(nullptr), numGpio(0),
    mcpConfigs(nullptr), numMcpDevices(0),
    lastMcpStates(nullptr), mcpStates(nullptr),
    mcpDebounceTime(nullptr), mcpInitialized(false),
...
/*
   Initializes MCP23017 device
*/
void SimRacingController::initializeMcp(uint8_t device) {
    if (device >= numMcpDevices) return;

    const McpConfig& config = mcpConfigs[device];

    // Reset the MCP23017
    writeMcpRegister(device, MCP23017_IOCONA, 0x00);
    writeMcpRegister(device, MCP23017_IOCONB, 0x00);

    // Set all pins as inputs
    writeMcpRegister(device, MCP23017_IODIRA, 0xFF);
    writeMcpRegister(device, MCP23017_IODIRB, 0xFF);

    // Enable pullups if configured
    if (config.usePullups) {
        writeMcpRegister(device, MCP23017_GPPUA, 0xFF);
    ...
    if (config.useInterrupts && config.intPin >= 0) {
        ...
        writeMcpRegister(device, MCP23017_GPINTENA, 0xFF);
    ...
    writeMcpRegister(device, MCP23017_IOCONA, 0x20);
}

/*
   I2C Communication with MCP23017
*/
...
bool SimRacingController::readMcpRegister(uint8_t device, uint8_t reg, uint8_t& value) {
    ...
    if (Wire.requestFrom(mcpConfigs[device].address, (uint8_t)1) != 1) return false;
    value = Wire.read();
    return true;
}
...
bool SimRacingController::readMcpPorts(uint8_t device, uint16_t& value) {
    ...
    if (Wire.requestFrom(mcpConfigs[device].address, (uint8_t)2) != 2) return false;
    value = Wire.read() | (Wire.read() << 8);
    return true;
}

/*
   Updates MCP23017 state
*/
void SimRacingController::updateMcp(uint8_t device) {
    ...
    if (!readMcpPorts(device, currentReading)) {
        lastError = ControllerError(ControllerError::MCP_ERROR, "Failed to read MCP ports");
        return;
    }
    ...
    for (int pin = 0; pin < 16; pin++) {
        ...
        if (pinState != ((lastMcpStates[device] >> pin) & 1)) {
    ...
    lastMcpStates[device] = currentReading;
}
...
/*
   Array initialization and cleanup
*/
...
void SimRacingController::setPowerSaveTimeout(unsigned long timeoutMs) {
